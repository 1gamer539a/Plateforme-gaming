...
import { ArrowLeft, Send, Sun, Moon, Store, Sparkles, Image as ImageIcon } from "lucide-react";
...
export default function FilDiscussion() {
  ...
  const envoyer = async () => {
    ...
    const { data, error } = await supabase
      .from("messages_chat")
      .insert({ conversation_id: conversationId, expediteur_id: userId, contenu })
      .select()
      .single();
    if (!error && data) {
      setMessages((prev) =>
        prev.some((m) => m.id === data.id) ? prev : [...prev, { id: data.id, role: "moi", texte: contenu, ia: false }]
      );
    }
  };
    ...
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col">
      ...
      >
        <div className="max-w-md mx-auto flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <button aria-label="Joindre une image"><ImageIcon size={18} color={COLORS.accentSecondary} /></button>
          <input
            value={saisie}
          ...
          />
          <button onClick={envoyer} aria-label="Envoyer"><Send size={18} color={COLORS.accentPrimary} /></button>
        </div>
      </div>
    </div>
  );
