...
export default function SalleSurveillance() {
  ...
  const [kpis, setKpis] = useState({ ca: 0, nbCommandes: 0, nbUtilisateurs: 0, topVendeurs: [], topProduits: [] });
  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      setKpis({ ca, nbCommandes: nbCommandes || 0, nbUtilisateurs: nbUtilisateurs || 0, topVendeurs, topProduits: [] });
  ...
  const validerIdentiteClient = async (signalementId, clientId) => {
    await supabase.from("users").update({ piece_identite_verifiee: true }).eq("id", clientId);
  ...
  const changerStatutVendeur = async (id, nouveauStatut) => {
    ...
    await supabase.from("vendeurs").update({ statut: nouveauStatut }).eq("id", id);
  };
        ...
        {onglet === "finances" && (
          <>
            <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              ...
              {userAjustement && (
                <div className="rounded-lg p-2 mb-2" style={{ background: COLORS.background, border: `1px solid ${COLORS.border}` }}>
                  <p className="text-xs font-semibold">{userAjustement.nom}</p>
                  <p className="text-[10px]" style={{ color: COLORS.textMuted }}>{userAjustement.telephone}</p>
                  <div className="flex gap-2 mt-2">
                    <input
